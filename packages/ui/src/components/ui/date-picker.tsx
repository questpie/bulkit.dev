...
import {
	Calendar,
	type CalendarProps,
} from "@bulkit/ui/components/ui/calendar/calendar";
import {
...
export function DatePicker(props: DatePickerProps) {
	...
	const [_month, _setMonthh] = useState<Date | undefined>(values?.month);
	...
	const hoursOptions = Array.from({ length: 24 }, (_, i) => ({
		value: String(i),
		label: hourFormat
			.format(setHours(startOfDay(new Date()), i))
			.padStart(2, "0"),
	}));
	const minutesOptions = Array.from({ length: 12 }, (_, i) => {
		...
		return {
			value: String(minute),
			label: minuteFormat
				.format(setMinutes(startOfDay(new Date()), minute))
				.padStart(2, "0"),
		};
	});
				...
				>
					<div className="flex justify-between items-center">
						...
						<div className="flex items-center flex-col gap-0">
							<CalendarMonthLabel />
							<CalendarControlTodayTrigger className="text-xs text-muted-foreground hover:text-primary cursor-pointer underline">
								Today
							</CalendarControlTodayTrigger>
						</div>
						...
						<div className="flex justify-center gap-1 items-center rounded-md border border-border ">
							<Select
								...
								onValueChange={(value) => {
									...
									props.onValueChange(
										setHours(new Date(currentDate), Number(value)),
									);
									setMinutesOpen(true);
								}}
							>
								<SelectTrigger
									className="w-auto underline border-none"
									hideCaret
								>
							...
							<Select
								...
								onValueChange={(value) => {
									const currentDate = values?.date ?? new Date();
									props.onValueChange(
										setMinutes(new Date(currentDate), Number(value)),
									);
								}}
							>
								<SelectTrigger
