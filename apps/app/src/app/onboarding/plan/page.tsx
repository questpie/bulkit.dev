...
export default async function PlanSelectionPage() {
	const sessionData = await apiServer.auth.session.get();
	...
	const plansResponse = await apiServer.plans.get();
			...
			<div className="max-w-5xl mx-auto p-4 py-16 space-y-12">
				<div className="text-center space-y-4 max-w-2xl mx-auto">
					...
					<p className="text-lg text-muted-foreground">
						Select the plan that best fits your needs. You can always change
						this later.
					</p>
				</div>
				...
				<PlanSelectionForm
					...
					initialPlans={plansResponse.data}
				/>
			</div>
		</main>
	);
