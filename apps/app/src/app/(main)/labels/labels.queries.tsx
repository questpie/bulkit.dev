...
export function labelsQueryOptions(filters?: LabelFilters) {
  return queryOptions({
    ...
    queryFn: async () => {
      const res = await apiClient.labels.index.get({
        query: filters || {},
      })
...
export function useCreateLabelMutation() {
  ...
  return useMutation({
    mutationFn: async (input: CreateLabelInput) => {
      const res = await apiClient.labels.index.post(input)
      if (res.error) throw res.error
      return res.data
    },
    onSuccess: (data) => {
...
export function useDeleteLabelMutation() {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      if (error.status === 400) {
        toast.error(error.value?.error || 'Cannot delete label - it is currently in use')
      } else {
...
export function useBulkLabelOperationMutation() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      ...
      toast.success(`Labels ${operationText} ${variables.resourceIds.length} resource(s)`)
    },
    onError: (error) => {
