...
export function tasksQueryOptions(filters: TaskFilters = {}) {
  return queryOptions({
    ...
    queryFn: async () => {
      const res = await apiClient.tasks.index.get({
        query: filters,
      })
      if (res.error) throw res.error
      return res.data
...
// Subtasks query
export function subtasksQueryOptions(parentTaskId: string, limit = 100, offset = 0) {
  return queryOptions({
...
export function useCreateTaskMutation() {
  ...
  return useMutation({
    mutationFn: async (input: CreateTaskInput) => {
      const res = await apiClient.tasks.index.post(input)
      if (res.error) throw res.error
      return res.data
    },
    onSuccess: () => {
...
export function useRemoveDependencyMutation() {
  ...
  return useMutation({
    mutationFn: async (dependencyId: string) => {
      const res = await apiClient.tasks.dependencies({ id: dependencyId }).delete()
      if (res.error) throw res.error
      return res.data
    },
    onSuccess: () => {
