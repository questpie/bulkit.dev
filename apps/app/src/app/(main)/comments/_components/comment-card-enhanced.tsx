...
export function CommentCardEnhanced({
	...
	const renderRichContent = (content: string) => {
		...
		// biome-ignore lint/security/noDangerouslySetInnerHtml: its ok
	...
	return (
		<div
			className={`${currentDepth > 0 ? "ml-8 border-l border-gray-200 pl-4" : ""}`}
		>
					...
					<div className="space-y-3">
						...
						<div className="flex items-start justify-between">
							<div className="flex items-center space-x-3">
								<Avatar className="h-8 w-8">
									<AvatarImage
										src={`https://avatar.vercel.sh/${comment.user.email}`}
									/>
						...
						{comment.reactions && comment.reactions.length > 0 && (
							<div className="flex flex-wrap gap-1">
								{Object.entries(comment.reactionCount || {}).map(
									([reactionType, count]) => {
										if (count === 0) return null;
										const emoji =
											reactionEmojis[
												reactionType as keyof typeof reactionEmojis
											];
										const color =
											reactionColors[
												reactionType as keyof typeof reactionColors
											];
										const userReacted = comment.reactions.some(
											(r) =>
												r.userId === comment.user.id &&
						...
						<div className="flex items-center space-x-4 pt-2">
							...
							<div className="flex items-center space-x-1">
								{["like", "love", "laugh"].map((reactionType) => {
									...
									const color =
										reactionColors[reactionType as keyof typeof reactionColors];
									const userReacted = comment.reactions.some(
										(r) =>
											r.userId === comment.user.id &&
											r.reactionType === reactionType,
