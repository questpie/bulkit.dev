...
export function PostsCalendar(props: {
	posts: Static<typeof ScheduledPostSchema>[];
}) {
	...
	const _setCurrentDate = (date: Date) => {
	...
	const { postMap, postEvents } = useMemo(() => {
		const scheduledPostsMap = new Map<
			string,
			Static<typeof ScheduledPostSchema>[]
		>();
	...
	const selectedPosts = selectedDate
		? postMap.get(getIsoDateString(selectedDate))
		: [];
					...
					>
						<Card className="w-full flex flex-col h-full justify-between gap-4 p-3">
							<div className="flex flex-row items-center gap-2 justify-between">
								...
								<ChannelAvatarList
									channels={scheduledPosts.map((p) => p.channel)}
									size="sm"
								/>
							</div>
