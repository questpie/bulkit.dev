...
import {
	Header,
	HeaderButton,
} from "@bulkit/app/app/(main)/_components/header";
...
export function PostDetailHeader({ post }: PostDetailHeaderProps) {
	...
	const publishMutation = useMutation({
		...
		onSuccess: (res) => {
			...
			if (res.error.status === 400) {
				...
				if (res.error.value.data.errors)
	...
	const returnToDraftMutation = useMutation({
		mutationFn: () =>
			apiClient.posts({ id: post.id })["return-to-draft"].patch(),
		onSuccess: (res) => {
			if (!res.error) {
	...
	const updateMutation = useMutation({
		mutationFn: (data: { name: string }) =>
			apiClient.posts({ id: post.id }).rename.patch(data),
		onSuccess: (res) => {
			if (!res.error) {
					...
					<DropdownMenu>
						...
						<DropdownMenuContent>
							...
							{post.status === "scheduled" && (
								<DropdownMenuItem
									onClick={() => returnToDraftMutation.mutate(undefined)}
								>
