...
export function PostFormProvider(props: PostFormProviderProps) {
	const form = useForm<Post>({
		...
		resolver: typeboxResolver(
			Type.Composite([
				PostDetailsSchema,
				getPostSchemaFromType(props.defaultValues.type),
			]),
		),
	});
	...
	const updateMutation = useMutation({
		mutationFn: apiClient.posts.put,
		onSuccess: (res) => {
...
export function RegularPostFields() {
	...
	const lastMediaOrder =
		mediaArray.fields[mediaArray.fields.length - 1]?.order ?? 1;
	...
	const handleDragEnd = (event: DragEndEvent) => {
		...
		if (over?.id && active.id !== over?.id) {
			const oldIndex = mediaArray.fields.findIndex(
				(item) => item.id === active.id,
			);
			const newIndex = mediaArray.fields.findIndex(
...
export function ThreadPostFields() {
	...
	const lastItemOrder =
		itemsArray.fields[itemsArray.fields.length - 1]?.order ?? 1;
