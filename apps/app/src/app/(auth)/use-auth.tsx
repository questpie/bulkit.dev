...
export type AuthData = Awaited<
	ReturnType<typeof apiClient.auth.session.get>
>["data"];
...
export const AuthProvider = ({
	...
	return (
		<AtomsHydrator atomValues={[[authAtom, authData]]}>
			{children}
		</AtomsHydrator>
	);
};
...
export function useAuthActions() {
	...
	const login = useMutation({
		mutationFn: async (token: string) => {
			const res = await apiClient.auth.session.post({
				authToken: token,
			});
	...
	const logout = useMutation({
		mutationFn: async () => {
			const res = await apiClient.auth.session.delete();
			if (res.error) {
				return null;
			}
