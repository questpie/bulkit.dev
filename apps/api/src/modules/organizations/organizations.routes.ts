		...
		async ({ auth, db, query }) => {
			...
			const baseQuery = db
				...
				.leftJoin(
					userRole,
					and(
						eq(userRole.organizationId, organizationsTable.id),
						eq(userRole.userId, auth.user.id),
					),
				)
				.leftJoin(
					aggregatedMembers,
					eq(aggregatedMembers.organizationId, organizationsTable.id),
				)
	...
	.get(
		...
		async (ctx) => {
			...
			const [members, total] = await Promise.all([
				membersQuery,
				ctx.db.$count(membersQuery),
			]);

			const nextCursor =
				members.length > ctx.query.limit
					? ctx.query.cursor + ctx.query.limit
					: null;
			return {
