...
export class PlansService {
	getLemonSqueezy() {
		if (envApi.DEPLOYMENT_TYPE !== "cloud") {
			throw HttpError.BadRequest(
				"Lemon Squeezy is not available in self-hosted version",
			);
		}
	...
	async getAvailablePlans(db: TransactionLike): Promise<AvailablePlan[]> {
		if (envApi.DEPLOYMENT_TYPE !== "cloud") {
			throw HttpError.BadRequest(
				"Plans are not available in self-hosted version",
			);
		}
		...
		const plans = await db
			...
			.where(
				and(
					eq(plansTable.status, "active"),
					isNotNull(plansTable.externalProductId),
				),
			)
		...
		return Promise.all(
			plans.map(async (plan) => {
				const variants =
					await this.getLemonSqueezy().getProductVariantsWithPrice(
	...
	async getActivePlanForOrganization(
		db: TransactionLike,
	...
	async getBillingPortalUrl(db: TransactionLike, organizationId: string) {
		const customer = await this.getLemonSqueezy().getCustomerForOrganization(
			db,
			organizationId,
		);
	...
	async createSubscription(
		...
		const { data: checkout } =
			await this.getLemonSqueezy().client.createCheckout(
...
export const injectPlanService = ioc.register(
	"planService",
	() => new PlansService(),
);
