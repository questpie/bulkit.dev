...
import {
	injectApiKeyManager,
	type ApiKeyManager,
} from "@bulkit/api/common/api-key.manager";
...
const MultiAgentStateAnnotation = Annotation.Root({
	...
	provider: Annotation<AITextProvider>({
		reducer: (_a: AITextProvider, b: AITextProvider) => b,
...
`);
		...
		return async (state: typeof MultiAgentStateAnnotation.State) => {
			...
			const supervisorSchema = z.object({
				nextAgent: z
					...
					.describe(
						"Which agent should handle this request next, or __end__ to finish",
					),
				reasoning: z
					.string()
					.describe("Brief reasoning for this routing decision"),
				priority: z
					.enum(["high", "medium", "low"])
					.describe("Priority level for this task"),
			});
			...
			if (state.context.pageContext) {
				...
				if (
					state.context.pageContext.entityType &&
					state.context.pageContext.entityId
				) {
	 ...
	 */
	async createCommentConversationGraph(
...
export const injectLangGraphAgentService = ioc.register(
