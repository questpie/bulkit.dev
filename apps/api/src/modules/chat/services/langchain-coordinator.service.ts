...
import {
	CHAT_CHANNELS,
	CHAT_EVENTS,
} from "@bulkit/shared/modules/chat/chat.constants";
import type {
	ChatMessage,
	PageContext,
	SmartReference,
...
import {
	...
	type LangGraphAgentService,
} from "./langgraph-agent.service";
...
export class LangChainCoordinatorService {
	...
	async *processUserMessageStream(
		...
		try {
			...
			const streamResult =
				await this.langGraphAgentService.streamMultiAgentConversation(
					opts.userMessage.content,
					context,
					provider,
				);
	...
	private async sendAgentMessage(
		...
		const userId = await this.organizationsService.ensureAIAssistant(
			db,
			organizationId,
		);
...
export const injectLangChainCoordinatorService = ioc.register(
	...
	(ioc) => {
		...
		return new LangChainCoordinatorService(
