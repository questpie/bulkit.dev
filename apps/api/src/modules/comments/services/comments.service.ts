...
import {
	injectMentionService,
	type MentionService,
} from "@bulkit/api/modules/mentions/mention.service";
import type {
	CommentListQuery,
	CommentListResponse,
	CommentReplyQuery,
...
export class CommentsService {
	...
	async createComment(
		...
		if (data.replyToCommentId) {
			...
			if (!parentComment) {
				throw HttpError.NotFound("Reply-to comment not found");
			}
		}

		// Extract mentions from content
		const mentions = this.mentionService.getMentionsFromHtml(data.content);
	...
	async updateComment(
		...
		if (existingComment.userId !== userId) {
			throw HttpError.Forbidden("Unauthorized to update this comment");
		}
	...
	async deleteComment(
		...
		if (existingComment.userId !== userId) {
			throw HttpError.Forbidden("Unauthorized to delete this comment");
		}
	...
	async getCommentList(
		...
		if (cursor) {
			whereConditions.push(
				gte(sql`extract(epoch from ${commentsTable.createdAt})`, cursor),
			);
		}
	...
	async addReaction(
		...
		const comment = await db
			...
			.where(
				and(
					eq(commentsTable.id, data.commentId),
					eq(commentsTable.organizationId, organizationId),
				),
			)
			.then((res) => res[0]);
	...
	private async updateReactionCount(
		db: TransactionLike,
		commentId: string,
	): Promise<void> {
		const reactions = await db
		...
		for (const reaction of reactions) {
			reactionCount[reaction.reactionType] =
				(reactionCount[reaction.reactionType] || 0) + 1;
		}
...
export const injectCommentsService = ioc.register("commentsService", (ioc) => {
	const { mentionService } = ioc.resolve([injectMentionService]);
	return new CommentsService(mentionService);
});
