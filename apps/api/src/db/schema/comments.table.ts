...
export type CommentMention = {
  type: 'user' | 'post' | 'media' | 'knowledge' | 'task'
  id: string
  name: string
  startIndex: number
  endIndex: number
...
export const commentsTable = pgTable(
  'comments',
  {
    ...
    // Reaction tracking
    reactionCount: jsonb('reaction_count').$type<Record<string, number>>().default({}),
...
export const commentAttachmentsTable = pgTable(
  'comment_attachments',
  {
    ...
    attachmentType: text('attachment_type', { enum: COMMENT_ATTACHMENT_TYPES }).notNull(),
...
export const commentReadStatusTable = pgTable(
  'comment_read_status',
  {
    ...
    userId: text('user_id')
      .notNull()
      .references(() => usersTable.id, { onDelete: 'cascade' }),
    readAt: timestamp('read_at', { mode: 'string', withTimezone: true }).defaultNow(),
  },
  (table) => ({
    commentIdIdx: index().on(table.commentId),
    userIdIdx: index().on(table.userId),
...
export type SelectCommentAttachment = typeof commentAttachmentsTable.$inferSelect
export type InsertCommentAttachment = typeof commentAttachmentsTable.$inferInsert
export type SelectCommentReadStatus = typeof commentReadStatusTable.$inferSelect
export type InsertCommentReadStatus = typeof commentReadStatusTable.$inferInsert
...
export const insertCommentReactionSchema = createInsertSchema(commentReactionsTable)
export const selectCommentReactionSchema = createSelectSchema(commentReactionsTable)
export const insertCommentAttachmentSchema = createInsertSchema(commentAttachmentsTable)
export const selectCommentAttachmentSchema = createSelectSchema(commentAttachmentsTable)
export const insertCommentReadStatusSchema = createInsertSchema(commentReadStatusTable)
export const selectCommentReadStatusSchema = createSelectSchema(commentReadStatusTable)
...
export const commentReadStatusRelations = relations(commentReadStatusTable, ({ one }) => ({
  comment: one(commentsTable, {
